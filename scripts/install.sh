...
for CUR_DIR in /host/${LOGDIR}/${NAME} /host/${DATADIR}/${NAME} /host/${CONFDIR}/${NAME} ; do
	...
	if [ "$CUR_DIR" == "/host/${CONFDIR}/${NAME}" ] ; then
	    #mv -v /evl-emu/config.yaml.example /host/${CONFDIR}/${NAME}/config.yaml
	fi
        chmod 775 $CUR_DIR
	chgrp -R 0 $CUR_DIR
	chmod g+r -R $CUR_DIR
...
echo 'KERNEL=="ttyUSB?", ATTRS{idVendor}=="0403", ATTRS{idProduct}=="6001", NAME="dsc0", SYMLINK="it100", GROUP="18", MODE="660"' > /host/etc/udev/rules.d/10-evl-emu.rules

chroot /host /usr/bin/udevadm control --reload-rules && chroot /host /usr/bin/udevadm trigger

chroot /host /usr/bin/podman create --name ${NAME} --net=host --device /dev/it100:/dev/it100,rw --entrypoint /sbin/entrypoint.sh -v ${DATADIR}/${NAME}:/var/lib/evl-emu:z,rw -v ${CONFDIR}/${NAME}:/etc/evl-emu:z,rw -v ${LOGDIR}/${NAME}:/var/log/evl-emu:z,rw ${IMAGE} /bin/start.sh
